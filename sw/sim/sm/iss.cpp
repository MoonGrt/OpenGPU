...
static uint32_t rr;
static uint32_t startup_pc = GPU_KERNEL_BASE;
static uint32_t launch_args_addr;
static uint32_t launch_args_size;
static uint32_t launch_grid[3] = {1, 1, 1};
static uint32_t launch_block[3] = {1, 1, 1};
static uint32_t launch_block_size = 1;
static bool faulted;
...
static inline uint32_t load_mem(gpu_iss_core_t *core, uint32_t addr, unsigned funct3) {
  switch (funct3) {
  case 0:
    return (uint32_t)sext(paddr_read(addr, 1), 8);
  case 1:
    return (uint32_t)sext(paddr_read(addr, 2), 16);
  case 2:
...
static inline void store_mem(gpu_iss_core_t *c, uint32_t addr, uint32_t value, unsigned funct3) {
  uint32_t mask;
  switch (funct3) {
  case 0:
    mask = 1;
    paddr_write(addr, 1, value);
    break;
  case 1:
    mask = 3;
...
static bool same_warp(const gpu_iss_core_t *left, const gpu_iss_core_t *right) {
  return left->warp_id == right->warp_id && left->block_idx[0] == right->block_idx[0] &&
         left->block_idx[1] == right->block_idx[1] && left->block_idx[2] == right->block_idx[2];
}
...
static void step_core(gpu_iss_core_t *c) {
  ...
  bool uses_rd = opcode == 0x37 || opcode == 0x17 || opcode == 0x13 || opcode == 0x33 ||
                 opcode == 0x03 || opcode == 0x6f || opcode == 0x67 || opcode == 0x73;
  bool uses_rs1 = opcode == 0x13 || opcode == 0x33 || opcode == 0x03 || opcode == 0x23 ||
                  opcode == 0x63 || opcode == 0x67;
  ...
  if ((uses_rd && rd >= 16) || (uses_rs1 && rs1 >= 16) || (uses_rs2 && rs2 >= 16)) {
    fault_core(c);
    return;
  }
  ...
  case 0x13: { // OP-IMM
    ...
    case 1:
      ...
      break;
    case 5:
      result = (funct7 == 0x20) ? (uint32_t)((int32_t)a >> ((inst >> 20) & 31))
                                : a >> ((inst >> 20) & 31);
      break;
    default:
      fault_core(c);
  ...
  case 0x33: // OP
    ...
    case 4:
      ...
      break;
    case 5:
      result = (funct7 == 0x20) ? (uint32_t)((int32_t)a >> (b & 31)) : a >> (b & 31);
      break;
    case 6:
      result = a | b;
      break;
    ...
    break;
  case 0x03:
    result = load_mem(c, a + sext(inst >> 20, 12), funct3);
    break;
  case 0x23: {
  ...
  case 0x63: {
    uint32_t imm = ((inst >> 7) & 0x1e) | ((inst >> 20) & 0x7e0) | ((inst << 4) & 0x800) |
                   ((inst >> 19) & 0x1000);
    bool take = false;
    switch (funct3) {
    case 0:
    ...
    if (take)
      next = c->pc + sext(imm, 13);
    break;
  }
  case 0x6f: {
    uint32_t imm = ((inst >> 20) & 0x7fe) | ((inst >> 9) & 0x800) | (inst & 0xff000) |
                   ((inst >> 11) & 0x100000);
    result = c->pc + 4;
  ...
  case 0x67: {
    ...
    for (uint32_t i = 0; i < ncores; ++i) {
      ...
      if (lane != c && !lane->halted && lane->pc == c->pc && same_warp(c, lane)) {
        uint32_t lane_target = (lane->gpr[rs1] + sext(inst >> 20, 12)) & ~1u;
        if (lane_target != next) {
          fault_core(c);
          return;
        }
  ...
  case 0x73:
    ...
    if (funct3 == 2 && rs1 == 0) {
      ...
      case 0xf14:
        ...
        break;
      case 0xcc0:
        result = c->thread_idx[0];
        break;
      case 0xcc1:
        result = c->thread_idx[1];
        break;
      case 0xcc2:
        result = c->thread_idx[2];
        break;
      case 0xcc3:
        result = c->block_idx[0];
        break;
      case 0xcc4:
        result = c->block_idx[1];
        break;
      case 0xcc5:
        result = c->block_idx[2];
        break;
      case 0xcc6:
        result = c->block_dim[0];
        break;
      case 0xcc7:
        result = c->block_dim[1];
        break;
      case 0xcc8:
        result = c->block_dim[2];
        break;
      case 0xcc9:
        result = c->grid_dim[0];
        break;
...
void gpu_iss_dcr_write(uint32_t addr, uint32_t data) {
  switch (addr) {
  case GPU_DCR_STARTUP_PC:
    startup_pc = data;
    break;
  case GPU_DCR_ARGS_ADDR:
    launch_args_addr = data;
    break;
  case GPU_DCR_ARGS_SIZE:
    launch_args_size = data;
    break;
  case GPU_DCR_BLOCK_DIM_X:
  case GPU_DCR_BLOCK_DIM_Y:
  case GPU_DCR_BLOCK_DIM_Z:
    launch_block[addr - GPU_DCR_BLOCK_DIM_X] = data;
    break;
  case GPU_DCR_GRID_DIM_X:
  case GPU_DCR_GRID_DIM_Y:
  case GPU_DCR_GRID_DIM_Z:
    launch_grid[addr - GPU_DCR_GRID_DIM_X] = data;
    break;
  case GPU_DCR_BLOCK_SIZE:
    launch_block_size = data;
    break;
  default:
    break;
  }
...
bool gpu_iss_launch(void) {
  ...
  uint64_t block_size = (uint64_t)launch_block[0] * launch_block[1] * launch_block[2];
  uint64_t total = blocks * block_size;
  if (total == 0 || total > UINT32_MAX || block_size != launch_block_size)
    return false;
  free(cores);
  cores = static_cast<gpu_iss_core_t *>(calloc(static_cast<size_t>(total), sizeof(*cores)));
  if (!cores)
    return false;
  ncores = (uint32_t)total;
  rr = 0;
  const uint32_t lanes_per_core = CONFIG_GPU_NUM_WARPS * CONFIG_GPU_NUM_THREADS;
  ...
  for (uint32_t i = 0; i < ncores; ++i) {
    ...
    cores[i].pc = startup_pc;
    cores[i].hartid = i;
    cores[i].physical_id = (block % hardware_cores) * lanes_per_core + local;
